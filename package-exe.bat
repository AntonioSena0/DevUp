...
del /q installer\DevUp-0.1.0.msi 2>nul
%JPACKAGE% --type msi --name DevUp --app-version 0.1.0 --vendor DevUp ^
  --description "DevUp" --copyright "DevUp" ^
  --input target --main-jar devup-0.1.0-all.jar ^
...
msiexec /i installer\DevUp-0.1.0.msi /passive
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
